# Dossier de conformité de publication — Maer Chat 0.5.4

> État au 28 août 2026. Ce dossier organise les preuves techniques et les
...
| --- | --- | --- |
| Version Android unique | Conforme dans les sources | `build.gradle`, version 0.5.4, code de base 9 (`900` universel, `904` ARM64) |
...
| Source correspondant au binaire | **Bloquant avant APK publique** | publier l’APK avec le tag immuable `v0.5.4` pointant sur son source exact |
...
| Qualité Spotless/Lint | Conforme pour Debug | 206 tests réussis ; Spotless et Lint Debug réussis sans nouveau constat. 626 constats historiques restent documentés dans la baseline ; la Release signée reste à qualifier |
...
5. créer le tag signé `v0.5.4` sur le commit compilé ;
